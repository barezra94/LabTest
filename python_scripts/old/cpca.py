...
class CPCA(object):
    ...
    def fit(self, foreground, background, preprocess_with_pca_dim=None):
        ...
        if not (self.features_d == self.features_d_bg):
            raise ValueError(
                "The dimensionality of the foreground and background datasets must be the same"
            )
        ...
        if self.features_d > preprocess_with_pca_dim:
            ...
            if self.verbose:
                print(
                    "Data dimensionality reduced to "
    ...
    def transform(
        ...
        if gui:
            ...
            def graph_foreground(ax, fg, active_labels, alpha):
                ...
                if alpha == 0:
                    ax.annotate(
                        r"$\alpha$=" + str(np.round(alpha, 2)) + " (PCA)",
                        (0.05, 0.05),
                        xycoords="axes fraction",
                    )
                else:
                    ax.annotate(
                        r"$\alpha$=" + str(np.round(alpha, 2)),
                        (0.05, 0.05),
                        xycoords="axes fraction",
                    )
            ...
            def update(value):
                ...
                for i in range(4):
                    ...
                    graph_foreground(
                        ax5, fg, self.active_labels, alphas_manual[alpha_idx]
                    )
    ...
    def automated_cpca(self, dataset, n_alphas_to_return, n_alphas, max_log_alpha):
        best_alphas, all_alphas, _, _ = self.find_spectral_alphas(
            n_alphas, max_log_alpha, n_alphas_to_return
        )
    ...
    def cpca_alpha(self, dataset, alpha=1):
        ...
        print("Data: ", reduced_dataset)
        print("Data Type: ", type(reduced_dataset))
    ...
    def my_cpca_alpha(self, dataset, alpha=1):
    ...
    def find_spectral_alphas(self, n_alphas, max_log_alpha, n_alphas_to_return):
        ...
        spectral = cluster.SpectralClustering(
            n_clusters=n_alphas_to_return, affinity="precomputed"
        )
        alphas = np.concatenate(([0], np.logspace(-1, max_log_alpha, n_alphas)))
...
class Kernel_CPCA(CPCA):
    def __init__(
        self, n_components=2, standardize=True, verbose=False, kernel="linear", gamma=10
    ):
        self.kernel = kernel
        self.gamma = gamma
    ...
    def fit_transform(
        ...
        if gui or plot:
            print(
                "The parameters gui and plot cannot be set to True in Kernel PCA. Will return transformed data as an array instead"
            )
        if not (alpha_selection == "manual"):
            print(
                "The alpha parameter must be set manually for Kernel PCA. Will be using value of alpha = 2"
            )
            alpha_value = 2
    ...
    def fit(self, foreground, background, preprocess_with_pca_dim=None):
        raise ValueError(
            "For Kernel CPCA, the fit() function is not defined. Please use the fit_transform() function directly"
        )
    ...
    def transform(
        ...
        raise ValueError(
            "For Kernel CPCA, the transform() function is not defined. Please use the fit_transform() function directly"
        )
    ...
    def centering(K, n):
        ...
        K_center[0:n, :][:, 0:n] = (
            Kx
            - np.ones([n, n]).dot(Kx) / n
            - Kx.dot(np.ones([n, n])) / n
            + np.ones([n, n]).dot(Kx).dot(np.ones([n, n])) / n / n
        )
        K_center[n:, :][:, n:] = (
            Ky
            - np.ones([m, m]).dot(Ky) / m
            - Ky.dot(np.ones([m, m])) / m
            + np.ones([m, m]).dot(Ky).dot(np.ones([m, m])) / m / m
        )
        K_center[0:n, :][:, n:] = (
            Kxy
            - np.ones([n, n]).dot(Kxy) / n
            - Kxy.dot(np.ones([m, m])) / m
            + np.ones([n, n]).dot(Kxy).dot(np.ones([m, m])) / n / m
        )
        K_center[n:, :][:, 0:n] = (
            Kyx
            - np.ones([m, m]).dot(Kyx) / m
            - Kyx.dot(np.ones([n, n])) / n
